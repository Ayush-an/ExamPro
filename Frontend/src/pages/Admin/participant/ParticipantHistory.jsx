...
import { fetchParticipantHistory } from "../../../utils/api";
...
export default function ParticipantHistory() {
  ...
  const loadHistory = async () => {
    setLoading(true);
    try {
      const res = await fetchParticipantHistory({
  ...
  const formatDetail = (detail) => {
    ...
    if (detail.groups?.length) parts.push(`Groups: ${detail.groups.length}`);
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-900 flex items-center gap-2">
            <Activity className="w-6 h-6 text-indigo-500" /> Participant History
          </h2>
          <p className="text-sm text-slate-500 mt-0.5">Audit log of all participant creations, edits, and bulk uploads.</p>
        </div>
