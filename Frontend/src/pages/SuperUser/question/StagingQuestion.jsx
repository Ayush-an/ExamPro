...
const StagingQuestion = () => {
  ...
  const loadInitial = async () => {
    ...
    try {
      ...
      toast.error("Staging telemetry synchronization failed");
    } finally {
      setLoading(false);
    }
  };
  ...
  const downloadExcel = () => {
    if (!examSummary.length) return toast.error("Empty summary matrix");
    ...
    XLSX.writeFile(wb, `StagingIntelligenceSummary_${new Date().toISOString().split('T')[0]}.xlsx`);
    toast.success("Staging intelligence audit exported");
  };

  const deleteAllQuestions = async (questionIds) => {
    if (!window.confirm("Confirm purge of all intelligence nodes for this protocol?")) return;
    try {
      ...
      toast.success("Intelligence purge successful");
      loadInitial();
    } catch (err) {
      toast.error("Intelligence purge Protocol Failure");
    }
  };
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center p-20 gap-4">
      ...
      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Scanning Intelligence Staging Area...</p>
    </div>
  );
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 text-left">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-slate-50">
        <div>
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Intelligence Staging Audit</h2>
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Verification of pending intelligence nodes before final synchronization</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-3 bg-slate-50 px-5 py-3 rounded-2xl border border-slate-100 min-w-[240px] shadow-inner">
            ...
            >
              <option value="">All Staged Protocols</option>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="p-8 bg-indigo-50 border border-indigo-100 rounded-[32px] flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold text-indigo-400 uppercase tracking-widest">Staged Protocols</p>
        ...
        <div className="p-8 bg-emerald-50 border border-emerald-100 rounded-[32px] flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest">Total Nodes</p>
        ...
        <div className="p-8 bg-amber-50 border border-amber-100 rounded-[32px] flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold text-amber-400 uppercase tracking-widest">Avg Nodes/Proto</p>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50/50 border-b border-slate-100">
              ...
              <th className="px-8 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Staged Protocol</th>
              <th className="px-8 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest text-center">Node Count</th>
              <th className="px-8 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Synchronization Attribution</th>
              ...
              <tr>
                <td colSpan="5" className="px-8 py-20 text-center">
                  <div className="flex flex-col items-center gap-4 opacity-20">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Staging Area Vacant</p>
                  </div>
                </td>
              </tr>
            ) : (
                ...
                <tr key={e.examId} className="hover:bg-slate-50/30 transition-colors group">
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{Array.from(e.groups).join(", ") || "Universal Access"}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6 text-center">
                    <span className="inline-flex px-4 py-1.5 bg-indigo-50 text-indigo-600 rounded-full text-[10px] font-black uppercase tracking-widest border border-indigo-100">
                      {e.totalQuestions} Nodes
                    </span>
                  </td>
                  <td className="px-8 py-6">
                    <div className="flex flex-col gap-1">
                      <span className="text-[10px] font-bold text-slate-900 uppercase tracking-widest">{e.createdBy}</span>
                  ...
                  <td className="px-8 py-6 text-right">
                    <button
                      ...
                      className="inline-flex items-center gap-2 px-5 py-2.5 bg-rose-50 text-rose-600 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-rose-500 hover:text-white transition shadow-sm border border-rose-100"
                    >
